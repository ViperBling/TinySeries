...
void triangle(Vec3f *pts, Vec2i *texts, float *zbuffer, TGAImage& image, float* intensity) {
    ...
    for (P.x = bboxmin.x; P.x <= bboxmax.x; ++P.x) {
        for (P.y = bboxmin.y; P.y <= bboxmax.y; ++P.y) {
            ...
            float ity;
            for (int i = 0; i < 3; ++i) ity += intensity[i];    // 取三个顶点的平均值
            ity /= 3;
...
Vec3f world2screen(Vec3f& v, Matrix& Projection, Matrix& ViewPort, Matrix& ModelView) {
...

    {
        ...
        for (int i = 0; i < model->nfaces(); ++i) {
            ...
            for (int j = 0; j < 3; j++) {
                ...
                screen_coords[j] = world2screen(world_coords[j], Projection, ViewPort, ModelView);
                // screen_coords[j] = world2screen(world_coords[j]);
                texts[j] = model->uv(i, j);
                // 根据法线方向和光线方向的夹角计算亮度
                Vec4f norm = embed<4>(model->norm(i, j));
                norm = ModelView * norm;
                Vec3f norm_proj = proj<3>(norm / norm[3]);
                intensity[j] = norm_proj * light_dir;
            }
